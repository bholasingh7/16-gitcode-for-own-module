#VPC
output "vpc_id" {
  value = aws_vpc.main.id
}

locals {
  # To format the subnet details in subnet_name = {id=..,az=..}
  public_subnet_output = {
    for key, config in local.public_subnet: key => {
      subnet_id = aws_subnet.main[key].id
      az = aws_subnet.main[key].availability_zone
    }
  }

  private_subnet_output = {
    for key, config in local.private_subnet: key => {
      subnet_id = aws_subnet.main[key].id
      az = aws_subnet.main[key].availability_zone
    }
  }
  direct_public_subnet = local.public_subnet
  direct_private_subnet = local.private_subnet
}

output "public_subnet_output" {
  value = local.public_subnet_output
}

output "private_subnet_output" {
  value = local.private_subnet_output
}


output "direct_public_subnet" {
  value = local.direct_public_subnet
}

output "direct_private_subnet" {
  value = local.direct_private_subnet
}